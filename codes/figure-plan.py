...
for k in range(1):#tqdm(range(len(sublinear_cell_lines[0])), desc='Cell line: '):#np.arange(0, 1):
        for n_ct in np.arange(5, 6):
            ...
            [x_all_list, x_ori_list, x_optim_list, error_plot_list, log_bias_list, log_bias_combined_list, log_bias_list_null, x_ori_list_null, x_optim_list_null,
...
for k in range(len(x_ori_list)):
    ...
    x_diff = x_optim_list[k] - x_ori_list[k]#x_list[-1] - x_list[0]
    i_change_all = np.where(x_diff != 0)[0]

    for i_change in i_change_all:
        changetype.append(np.where(x_diff[i_change] > 0, 'Added', 'Removed'))

        if i_change < max_links:
            linktype.append('Consumption')
            ct = i_change//MAX_ID_metabolites
            # l1, m1 = calculate_overlap_stats(x_ori_list[k], n_ct, max_links, met_ID)[:2]
            l2, m2 = calculate_overlap_stats(x_optim_list[k], n_ct, max_links, met_ID)[:2]
    ...
    df = pd.DataFrame({'LinkType': linktype,
...
for k in range(len(x_ori_list_null)):
    linkchange = []
    linktype = []
    changetype = []
    test = []
    ...
    # for i in range(1, len(x_list)):
    x_diff = x_optim_list_null[k] - x_ori_list_null[k]#x_list[-1] - x_list[0]
    i_change_all = np.where(x_diff != 0)[0]
    ...
    for i_change in i_change_all:
        ...
        if i_change < max_links:
            linktype.append('Consumption')
            ct = i_change//MAX_ID_metabolites
            # l1, m1 = calculate_overlap_stats(x_ori_list_null[k], n_ct, max_links, met_ID)[:2]
            l2, m2 = calculate_overlap_stats(x_optim_list_null[k], n_ct, max_links, met_ID)[:2]
    ...
    #                     'Link': [str(arr[0]) for arr in list(compress(linkchange, i_valid))],
...
g = sns.histplot(data=pooled_df, x='OverlapStatus',
                ...
                legend=False, shrink=0.8, discrete=True)
