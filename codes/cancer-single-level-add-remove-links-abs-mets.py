...
File: one-celltype-add-links
-----------------------------
Author: akshitgoyal, bvibishan
Data:   28/03/2025, 09:32:05
---------------------------->
"""
# %%
import pandas as pd
...
def run_network_model(f, col_name, net, x, cellnum_init, cellnum_max, MAX_ID_celltypes, MAX_ID_metabolites):
    ...
    # fig.savefig('../figures/one-celltype-add-remove-links/absolute-abundance/ec-corr-%d.png' % (f_count), dpi=300.)
...
def compute_met_matrices(cellnum_init, diet, net, MAX_ID_celltypes, MAX_ID_metabolites):
    ct0 = cellnum_init*celltypefreq.to_numpy() # Prior cell type relative frequencies
    # b_real[metagenome_ID] = metagenome.iloc[:,pa] / np.sum(metagenome.iloc[:,pa])

    ######## Assign diet using 0h extracellular measurement from Jain et al.
    x = np.clip(diet, 0, np.inf) # Initial absolute abundances of metabolites in the diet
...
for i, f in enumerate(f_arr):
    for j, cl in enumerate(cell_line_names):
        ...
        m2b, b2m = compute_met_matrices(cellnum_init_all[j], diet, net, MAX_ID_celltypes, MAX_ID_metabolites)
        ...
        fun = lambda x: pred_error_addingLinks(x, m2b_ori, b2m_ori, x_ori, net_ori, f, cl, diet, cellnum_init_all[j], cellnum_final_all[j], i_nonzero_metabolites)
        ...
        ###### Record model performance with links added
        ec_corr[i, j], ct_full[i, j], met_meas, met_pred, i_common = run_network_model(f, cell_line_names[j], net_new)
# %%
